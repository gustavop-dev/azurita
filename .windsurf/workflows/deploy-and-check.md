---
description: Deploy latest master to production server for azurita
---

# Deploy azurita to Production

Run these steps on the production server at `/home/ryzepeck/webapps/azurita` to deploy the latest `master` branch.
...
```bash
cd /home/ryzepeck/webapps/azurita && git pull origin master
```
...
- **Backend**: Django (`azurita_project` module), settings via `DJANGO_ENV=production` auto-import

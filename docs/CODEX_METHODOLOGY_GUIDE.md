# Azurita Codex Methodology Guide

Azurita uses a **Codex-native repo runtime**. Claude Code and Windsurf are maintained as compatibility layers only — they must stay aligned with the Codex-native source of truth, not redefine it.
...
| `plugin` | Optional packaging format for sharing skills *across* repos (not Azurita's daily runtime) |

...
> **Azurita override**: the only existing view (`azurita_project.views.index`) is a plain function view that renders `templates/index.html`. There are **no DRF endpoints yet**. When adding APIs, prefer **function-based views with `@api_view`** for consistency. Do not introduce CBVs/APIView unless explicitly requested.
...
### Execution Rules
1. **Activate virtual environment** before any command. Azurita's venv lives at the **repo root** (not `.venv/`, but `venv/`):
   - From repo root: `source venv/bin/activate`
   - `manage.py` is at the **repo root**, so most commands run from there directly.
2. **Run only modified test files**: `pytest backend/path/to/test_file.py -v`. The root `pytest.ini` sets `DJANGO_SETTINGS_MODULE=azurita_project.settings` and adds `--cov=azurita_project --cov-branch` by default — pass `--no-cov` for fast iteration.
